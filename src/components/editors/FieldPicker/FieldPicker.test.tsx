...
describe('FieldPicker', () => {
  ...
  it('Should allow to select field', () => {
    ...
    fireEvent.change(selectors.root(), { target: { values: 'A:name' } });
  ...
  it('Should allow to clean value', () => {
    render(getComponent({ value: { source: 1, name: 'value' }, isClearable: true }));
  ...
  it('Should allow to select fields based on already selected', () => {
    ...
    fireEvent.change(selectors.root(), { target: { values: 'A:value' } });

    expect(onChange).toHaveBeenCalledWith({ name: 'value', source: 'A' });
  });

  it('Should filter all fields by type', () => {
    render(getComponent({ includeTypes: [FieldType.number] }));

    fireEvent.change(selectors.root(), { target: { values: '1:value' } });
  ...
  it('Should filter selected fields by type', () => {
    ...
    fireEvent.change(selectors.root(), { target: { values: '1:name' } });
